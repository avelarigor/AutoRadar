...
# -*- coding: utf-8 -*-
"""
Coleta de Links da Webmotors
Extrai links de anúncios da página de busca da Webmotors.
...
DEFAULT_WEBMOTORS_URL = "https://www.webmotors.com.br/carros-usados/mg-montes-claros?lkid=1000&tipoveiculo=carros-usados&localizacao=-16.7286406%2C-43.8582139x100km&estadocidade=Minas%20Gerais-Montes%20Claros&page=1"

# Chrome existente (igual ao Facebook) — quando disponível usa a mesma sessão
CDP_URL = "http://127.0.0.1:9222"

# User-Agent e viewport mobile (evita desafio "Pressione e segure" na versão desktop)
MOBILE_USER_AGENT = (
...
def parse_webmotors_url(search_url: str) -> dict:
    """
    Parseia o link da Webmotors e extrai componentes para construir URLs de busca.
    Retorna dict com: base_url, path, query_params, page_param_name
    """
    if not search_url or not search_url.strip():
        search_url = DEFAULT_WEBMOTORS_URL
    ...
    try:
        ...
        _log("Erro ao parsear URL da Webmotors: %s" % e)
        # Fallback para URL padrão
        return parse_webmotors_url(DEFAULT_WEBMOTORS_URL)
...
def collect_links(
    ...
    """
    Coleta links de anúncios da Webmotors.
    ...
    url_info = parse_webmotors_url(search_url)
    links = []
    seen_links = set()
    
    _log("Coleta: iniciando Webmotors (URL base: %s)" % url_info["base_url"])
    
    if status_message_callback:
        status_message_callback("Coletando links da Webmotors...")
    ...
    def _run_with_browser(browser_instance, close_browser_when_done: bool):
        ...
        _log("Coleta Webmotors: usando versão mobile (viewport %s) para evitar desafio de bot" % MOBILE_VIEWPORT)
        try:
            ...
            for page_num in range(1, max_pages + 1):
                ...
                try:
                    # Usar domcontentloaded primeiro (evita timeout de 60s com networkidle em sites pesados)
                    try:
                        page.goto(url, wait_until="domcontentloaded", timeout=45000)
                    except Exception as goto_error:
                        error_str = str(goto_error).lower()
                        if "504" in error_str or "502" in error_str or "503" in error_str or "cloudfront" in error_str or "timeout" in error_str:
                            _log("Coleta Webmotors: erro (%s). Tentando load..." % error_str[:80])
                            page.goto(url, wait_until="load", timeout=30000)
                        else:
                            raise
                    # Esperar os cards de anúncios (seletor de anúncio ou listagem)
                    try:
                        page.wait_for_selector('a[href*="/comprar/"]', timeout=6000)
                    except Exception:
                        try:
                            page.wait_for_selector('a[href*="/carros-usados/"]', timeout=4000)
                        except Exception:
                            pass
                    try:
                        ...
                        time.sleep(0.2)
                    # Jitter leve anti-block (150–450 ms)
                    try:
                        page.wait_for_timeout(random.randint(150, 450))
                    except Exception:
                        time.sleep(random.uniform(0.15, 0.45))
                    # Buscar links de ANÚNCIOS (página de detalhe: /comprar/marca/modelo/.../ID)
                    # Não coletar links de categoria/listagem (ex: .../mg-montes-claros, .../carros-usados/mg-montes-claros/chevrolet)
                    page_links = page.evaluate("""
                            () => {
                                const links = [];
                                const seen = new Set();
                                const base = 'https://www.webmotors.com.br';
                                function add(href) {
                                    if (!href) return;
                                    if (!href.startsWith('http')) href = base + (href.startsWith('/') ? href : '/' + href);
                                    const pathParts = href.split('?')[0].split('/').filter(Boolean);
                                    // Só adicionar links que são página de anúncio: /comprar/marca/modelo/versao/.../ID (path longo e termina com número)
                                    const isComprar = href.includes('/comprar/');
                                    const pathLongEnough = pathParts.length >= 6;
                                    const endsWithId = /\\d+$/.test(pathParts[pathParts.length - 1]);
                                    const notSearchPage = !href.includes('mg-montes-claros') || pathParts.length >= 7;
                                    if (!seen.has(href) && isComprar && pathLongEnough && (endsWithId || pathParts.length >= 7) && notSearchPage) {
                                        seen.add(href);
                                        links.push(href);
                                    }
                                }
                                // 1) Links /comprar/... (página de anúncio)
                                document.querySelectorAll('a[href*="/comprar/"]').forEach(el => {
                                    const h = el.getAttribute('href');
                                    if (h) add(h);
                                });
                                // 2) Fallback: vehicle card com link para anúncio
                                if (links.length === 0) {
                                    const containers = document.querySelectorAll('[data-testid="vehicle_card_oem_container"]');
                                    for (const c of containers) {
                                        const a = c.querySelector('a[href*="/comprar/"]') || c.querySelector('a[href*="/quero-negociar"]') || c.querySelector('a[href*="/carros-usados/"]');
                    ...
                    if len(new_links) == 0:
                        ...
                        break
                    # Jitter entre páginas (150–450 ms)
                    try:
                        page.wait_for_timeout(random.randint(150, 450))
                    except Exception:
                        time.sleep(random.uniform(0.15, 0.45))
                    ...
                    _log("❌ Coleta Webmotors: erro na página %d - %s" % (page_num, str(e)[:200]))
                    if "504" in error_str or "cloudfront" in error_str:
                        _log("⚠️ Erro 504 CloudFront: servidor sobrecarregado ou indisponível. Continuando pipeline sem Webmotors.")
                    elif "502" in error_str or "503" in error_str:
                        _log("⚠️ Erro %s: servidor temporariamente indisponível. Continuando pipeline sem Webmotors." % error_str[:10])
                    elif "timeout" in error_str or "timed_out" in error_str:
                        _log("⚠️ Timeout: conexão lenta ou servidor não respondeu. Continuando pipeline sem Webmotors.")
                    elif "net::" in error_str or "network" in error_str:
                        _log("⚠️ Erro de rede: problema de conexão. Continuando pipeline sem Webmotors.")
                    else:
                        _log("⚠️ Erro desconhecido. Continuando pipeline sem Webmotors.")
                    break
            ...
            with sync_playwright() as p:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    if browser is None:
                        raise RuntimeError("collect_links_webmotors.py: CDP indisponível e fallback Chromium desativado.")
