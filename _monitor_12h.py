...
Suporte a crossing de meia-noite: ajusta o caminho do log automaticamente.
"""
...
OUT_PATH   = Path("logs/monitor_12h_report.log")
INTERVAL   = 10 * 60        # 10 minutos
DURATION   = 12 * 60 * 60   # 12 horas

APP_PID_FILE = Path("logs/app_pid.txt")

def get_log_path():
    """Retorna o caminho do log do dia atual (rotativo por data)."""
    return Path("logs") / f"autoradar_{datetime.now().strftime('%Y%m%d')}.log"
...
def check_log_activity(last_size, log_path):
...
def db_snapshot():
    try:
        ...
        cur.execute("SELECT COUNT(*) FROM opportunities WHERE sent_at IS NOT NULL")
        sent = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM opportunities WHERE sent_at IS NULL")
        pending = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM opportunities WHERE fipe_price IS NOT NULL AND fipe_price > 0")
        with_fipe = cur.fetchone()[0]
        cur.execute("""
            SELECT id, title, price, fipe_price, fipe_margin, source
...
def run_monitor():
    log("=" * 60)
    log("MONITOR 12H — AutoRadar iniciado")
    ...
    current_log_path = get_log_path()
    last_log_size = current_log_path.stat().st_size if current_log_path.exists() else 0
    ...
    while time.time() < end_time:
        ...
        # ── Checar rotação de log após meia-noite ──────────
        today = datetime.now().date()
        if today != current_date:
            log(f"[LOG] Novo dia detectado ({today}) — rotacionando caminho do log")
            current_log_path = get_log_path()
            last_log_size = 0  # reset para o novo arquivo
            current_date = today
        ...
        # ── 2. Atividade no log ────────────────────────────
        curr_size, new_lines, is_active = check_log_activity(last_log_size, current_log_path)
    ...
    # ── RESUMO FINAL ──────────────────────────────────────
    log("\n" + "=" * 60)
    log("RESUMO FINAL — 12 HORAS DE MONITORAMENTO")
    log("=" * 60)
